...
WEIGHT_DIR = os.path.abspath("/content/gdrive/My Drive/Mask/")
annotations1 = json.load(open(os.path.join(WEIGHT_DIR, "via_export_json.json")))
annotations = list(annotations1.values())
length_to_split = [2000,len(annotations)-2000]
random.shuffle(annotations)
Inputt = iter(annotations) 
O = [list(islice(Inputt, elem)) for elem in length_to_split]
# Import Mask RCNN
...
COCO_WEIGHTS_PATH = os.path.join(WEIGHT_DIR, "mask_rcnn_coco.h5")
...
class CustomConfig(Config):
    ...
    #with 12GB memory, which can fit two images.
...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        self.add_class("object", 1, "gun")
        self.add_class("object", 2, "knife")
        self.add_class("object", 3, "scissor")
        ...
        # Add images
            for a in annotations:
                ...
                name_dict = {"gun": 1,"knife": 2,"scissor": 3}
                # key = tuple(name_dict)
